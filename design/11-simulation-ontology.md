...
**Objects become historically significant through events.** An Object's historical-significance tier is *computed*, on demand, from its currently-active high-interest Records — never stored as a permanent flag (per the engine-wide law in `18-prime-movers.md` §"Significance is derived, never stored": significance decays without reinforcement, exactly like opinion, standing, and belief). Historically significant Objects appear in Chronicles, Annals, and Legends. The threshold, and how quickly significance fades absent new events, are pack-defined.
...
- **Reducers** — `computeOpinion`, `computeStanding`, `computeBelief`, `computeStatusBelief`, `orgBeliefOf` — are **pure reads**. Each answers one question ("how do I feel about them?", "is P believed?", "who holds this slot?") and mutates nothing: a reducer never adds a Mark and never calls a producer. **A reducer may depend only on reducers closer to the substrate — never on a peer or a consumer.** A higher reducer may build on a lower one (`computeStatusBelief` resolves over `computeBelief`, a *radius* toward `mark`); it may never reach sideways to a sibling. This defines the graph rather than naming today's functions — with eight reducers it still holds, and the reducers never form a web.
...
| 1.5 | 2026-07-03 | Tightened the reducer-dependency rule to a graph-defining invariant (*a reducer may depend only on reducers closer to the substrate — never a peer or consumer*), which scales past today's four functions. Purged the sole lateral edge in code: `belief.ts` no longer imports `opinion.ts` — `tellBelief`/`shareBelief` take `sourceTrust` as a parameter and the orchestration layer (`resolve.ts`) derives it via `opinion.trustFromOpinion`, so trust POLICY lives at the caller (swappable for rank/sensor/attunement) and the subjective graph is now fully radial. |
| 1.6 | 2026-07-12 | Reworded §Object's "historically significant" language from a one-time "flagged" promotion to an explicitly *computed, decaying* tier, aligned with the engine-wide law added the same day in `18-prime-movers.md` ("Significance is derived, never stored") — a fix made pre-emptively, since Object is not yet implemented in the PoC and this closes the gap before code exists to violate it. |
